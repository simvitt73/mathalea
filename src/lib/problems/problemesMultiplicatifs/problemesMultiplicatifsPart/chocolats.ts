...
export function chocolats(decimal = false): Probleme {
  ...
  probleme.correction = `On cherche à répartir $${texNombre(total, 2)}$ ${unité} dans ${nbFois} boites. Donc la masse qu'on doit mettre dans chaque boite est : $${texNombre(total, 2)}\\text{\\,${unité}} \\div ${texNombre(nbFois, 2)}=${miseEnEvidence(texNombre(total / nbFois, 2))}\\text{\\,${unité}}$.`
