  ...
  nouvelleVersion () {
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      do {
        ...
        c = AB1 ** 2 + AB2 ** 2 + AB3 ** 2
        delta = b ** 2 - 4 * a * c
      } while ((AB2 * u3 - AB3 * u2 === 0 && AB3 * u1 - AB1 * u3 === 0 && AB1 * u2 - AB2 * u1 === 0) || delta > 0 || b === 0) // On recommence si AB et u sont colinéaires ou si on a un discriminent négatif, signe que la fonction n'est pas strictement positive.
      ...
      const question1 = 'Exprimer $AM$ en fonction de $t$ en détaillant les calculs.'// + ajouteQuestionMathlive({ exercice: this, question: 0, objetReponse: { reponse: { value: `\\sqrt{${exprF}}`.replaceAll('x', 't') } }, typeInteractivite: 'mathlive', texteAvant: ' $AM(t)=$' })
      const correction1 = `On a :<br>$\\begin{aligned}AM^2 &=\\left(x_A-x_M\\right)^2+\\left(y_A-y_M\\right)^2+\\left(z_A-z_M\\right)^2\\\\
     ...
     D'où $AM=\\sqrt{${exprF.replaceAll('x', 't')}}$`.replaceAll('-0', '').replaceAll('+0', '').replaceAll('(0+', '(').replaceAll('(0-', '(') // ça c'est pour éviter d'avoir des +0 ou -0 dans les réponses

      let question2 = `On considère la fonction $f$ définie par $f(x) = \\sqrt{${exprF}}$.<br>`
      const question2a = 'Justifier que $f$ est définie et dérivable sur $\\mathbb{R}$ puis calculer $f\'(x)$ pour tout x.'// + ajouteQuestionMathlive({ exercice: this, question: 1, objetReponse: { reponse: { value: `\\frac{${2 * a}x${ecritureAlgebrique(b)}}{2\\sqrt{${exprF}}}` } }, typeInteractivite: 'mathlive', texteAvant: ' $f^\\prime(x)=$' })
      ...
      const fonctionF = (x:number | FractionEtendue) => Math.sqrt(a * Number(x) ** 2 + b * Number(x) + c)
      const deriveeF = (x:number | FractionEtendue) => (2 * a * Number(x) + b) / (2 * fonctionF(x))
      const tableau = tableauVariationsFonction(fonctionF, deriveeF, -10, 10, {
        ligneDerivee: true,
        substituts: [
          { antVal: -10, antTex: '-\\infty', imgTex: '+\\infty' },
          { antVal: 10, antTex: '+\\infty', imgTex: '+\\infty' },
          { antVal: -b / 2 / a, antTex: xMinimum.texFSD, imgTex: `$f(${xMinimum.texFSD.replace('dfrac', 'frac')})$` }
        ]
      })

      const question2b = 'Montrer que $f$ admet un minimum en une valeur $\\mu$ que l\'on déterminera.'// + ajouteQuestionMathlive({ exercice: this, question: 2, objetReponse: { reponse: { value: `\\frac{-${AB1 * u1 + AB2 * u2 + AB3 * u3}}{${u1 ** 2 + u2 ** 2 + u3 ** 2}}` } }, typeInteractivite: 'mathlive', texteAvant: ' $x_0=$' })
    ...
    &=${minimum}\\end{aligned}$<br>`
      ...
      const listeC2 = createList({
        items: [
          correction2a, correction2b, correction2c
        ],
        style: 'alpha'
      })
      ...
      const question3 = 'En déduire les coordonnées du projeté orthogonal de $A$ sur $(d)$.'
    ...
    D'après l'étude de la fonction précédente, les coordonnées de $M$ sont données par la valeur minimale $t=${xMinimum.texFSD}$ :<br>`
      ...
      if (this.questionJamaisPosee(i, x0, y0, z0, u1, u2, u3)) {
        this.listeQuestions[i] = texte + createList({
          items: [
