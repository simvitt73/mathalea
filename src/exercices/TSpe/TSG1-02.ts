...
export default class NomExercice extends ExerciceSimple {
  ...
  nouvelleVersion () {
    ...
    let kuplet = ''
    if (n === 2) { kuplet = 'couples' }
    if (n === 3) { kuplet = 'triplets' }
    if (n === 4) { kuplet = 'quadruplets' }
    if (n === 5) { kuplet = 'quintuplets' }
    if (n >= 6) { kuplet = `${n}-uplets` }
    const factorielleN = factorielle(n)
    const prenom = choice(['Raoul', 'Simone', 'Lucien', 'Bernadette', 'Lucienne', 'Edmond', 'Roger', 'Cunégonde'])
    const objet = choice(['livres', 'timbres', 'cartes', 'cahiers', 'carnets', 'CD', 'boîtes'])
