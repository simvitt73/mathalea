...
export default class ExerciceNumerationEntier extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, texte, texteCorr, a, b, rangA, rangB, rangRef, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      const nbReponse = this.sup2 ? b * Math.pow(10, rangB) + a * Math.pow(10, rangA) : b * Math.pow(10, rangB - rangRef) + a * Math.pow(10, rangA - rangRef)
      ...
      if (context.isAmc) {
        ...
        this.autoCorrection[i] = {
          ...
          reponse: {
            ...
            param: {
              digits: nbDigitsSupplementaires + nombreDeChiffresDansLaPartieEntiere(nbReponse), // obligatoire pour AMC (le nombre de chiffres pour AMC, si digits est mis à 0, alors il sera déterminé pour coller au nombre décimal demandé)
