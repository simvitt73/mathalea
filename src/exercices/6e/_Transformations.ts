  ...
  nouvelleVersion () {
    ...
    for (let ee = 0, texte, texteCorr, xu, yu, pointMLettre, pointM, pointN, numPointN, croix, aEviter, mauvaisAntecedents, longueurBoucle, objetsEnonce, objetsCorrection, cpt = 0; ee < this.nbQuestions && cpt < 50;) {
      ...
      for (let i = 0, labO, labM, labN, traceAnt, traceIm, traceO, traceM, traceN; i < nbImages; i++) {
        ...
        traceO.epaisseur = 2
        traceO.opacite = 1
        labO = labelLatexPoint({ points: [O], color: 'red', taille: 10, couleurDeRemplissage: 'white' })
        switch (choixTransformation[i]) {
          ...
          case 8:
            ...
            traceM.epaisseur = 1
            traceN.epaisseur = 1
            labM = labelLatexPoint({ points: [pointM], color: couleurs[i], taille: 8, couleurDeRemplissage: 'white' })
            labN = labelLatexPoint({ points: [pointN], color: couleurs[i], taille: 8, couleurDeRemplissage: 'white' })
