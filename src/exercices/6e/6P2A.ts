...
export default class PlacerProbabilitesV2 extends Exercice {
  constructor () {
    ...
    this.besoinFormulaireNumerique = ['Graduation de l\'axe', 3, '1 : Des valeurs numériques\n2 : Des mots\n3 : Des valeurs numériques et des mots']
    this.nbQuestions = 1
  ...
  nouvelleVersion () {
    ...
    const lstEchelle:[string, number, string][] = [
      ['Impossible', 0, texteEnCouleurEtGras('égale à ') + `$${miseEnEvidence('0')}$`],
      ['Improbable', 1 / 6, texteEnCouleurEtGras('proche de ') + `$${miseEnEvidence('0')}$`],
      ['Peu probable', 2 / 6, texteEnCouleurEtGras('inférieure à ') + `$${miseEnEvidence('\\dfrac12')}$`],
      ['Une chance sur deux', 3 / 6, texteEnCouleurEtGras('égale à ') + `$${miseEnEvidence('\\dfrac12')}$`],
      ['Probable', 4 / 6, texteEnCouleurEtGras('supérieure à ') + `$${miseEnEvidence('\\dfrac12')}$`],
      ['Très probable', 5 / 6, texteEnCouleurEtGras('proche de ') + `$${miseEnEvidence('1')}$`],
      ['Certain', 1, texteEnCouleurEtGras('égale à ') + `$${miseEnEvidence('1')}$`]
    ]
    ...
    for (let indice = 0; indice < lstObjet.length; indice++) { lstObjetCorr.push(lstObjet[indice]) }
    ...
    if (this.sup === 2) {
      ...
      lstObjet.push(latex2d('\\dfrac{1}{2}', L / 2, 0.75, { letterSize: 'scriptsize' }))
      lstObjet.push(latex2d('0', 0, 0.75, { letterSize: 'scriptsize' }))
      lstObjet.push(latex2d('1', L, 0.75, { letterSize: 'scriptsize' }))
      lstObjetCorr.push(latex2d('\\dfrac{1}{2}', L / 2, 1.15, { letterSize: 'scriptsize' }))
    ...
    for (let i = 0; i < nbEvenement; i++) {
      ...
      lstObjetCorr.push(texteParPosition(txtSolution, L * p, ylst[parrondi], 0, 'black', 1, 'milieu'))
    ...
    for (let i = 0; i < nbEvenement; i++) {
      ...
      texteCorr += String.fromCharCode(65 + i) + ' : ' + lstEvenenementExo[i][0] + '.<br>'
      texteCorr += texteEnCouleurEtGras(lstEchelle[parrondi][0]) + ` de se produire, cet événement a une probabilité ${lstEchelle[parrondi][2]}.<br>`
    }
    if (context.isHtml) {
    ...
    texteCorr += mathalea2d({ xmin: -1, xmax: L + 3, ymin: miny, ymax: 2, pixelsParCm: 40, scale: 1 }, lstObjetCorr)
    if (context.isHtml) {
      texteCorr += '</p>'
    } else {
