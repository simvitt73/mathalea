  ...
  nouvelleVersion () {
    ...
    for (let i = 0,
      ...
      if (!div && typesDeQuestions < 4) {
        ...
        texteCorr += '<br>' + buildTab(String(a), prefixeMulti[k][0] + 'm', String(resultat), unite, 2, true)
        ...
        texteCorr += '<br>' + buildTab(String(a), prefixeDiv[k][0] + 'm', String(resultat), unite, 2, true)
      } else if (typesDeQuestions === 4) {
        ...
        if (randint(0, 1) > 0) {
          ...
          texteCorr += '<br>' + buildTab(String(a), listeUnite[unite2], String(resultat), listeUnite[unite1], 2, true)
        } else {
          ...
          texteCorr =
                        ...
                        '$'
          prefixe = Math.pow(10, 2 * ecart)
          texteCorr += '<br>' + buildTab(String(a), listeUnite[unite1], String(resultat), listeUnite[unite2], 2, true)
        }
      } else {
        prefixeMulti = [
        ...
        texteCorr += '<br>' + buildTab(String(a), String(prefixeMulti[k][0]), String(resultat), unite, 2, true, false, true)
      }
...
function buildTab (nombre1:string, uniteNombre1:string, nombre2:string, uniteNombre2:string, ligne = 2, force = false, correction = false, hectare = false) {
  const tabRep = function (nbre: string, uniteNbre:string): string[] {
    const res = ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '']
    switch (uniteNbre.replaceAll(' ', '')) {
      case 'km':
        for (let i = 0; i <= 21; i++) {
          res[i] = (5 - i === 0 ? '\\color{red}{' : '') + getDigitFromNumber(nbre, Decimal.pow(10, 5 - i).toNumber()) + (5 - i === 0 ? (new Decimal(nbre).decimalPlaces() === 0 ? '}' : ',}') : '')
        }
        break
      case 'hm':
      case 'ha':
        for (let i = 0; i <= 21; i++) {
          res[i] = (7 - i === 0 ? '\\color{red}{' : '') + getDigitFromNumber(nbre, Decimal.pow(10, 7 - i).toNumber()) + (7 - i === 0 ? (new Decimal(nbre).decimalPlaces() === 0 ? '}' : ',}') : '')
        }
        break
      case 'dam':
      case 'a':
        for (let i = 0; i <= 21; i++) {
          res[i] = (9 - i === 0 ? '\\color{red}{' : '') + getDigitFromNumber(nbre, Decimal.pow(10, 9 - i).toNumber()) + (9 - i === 0 ? (new Decimal(nbre).decimalPlaces() === 0 ? '}' : ',}') : '')
        }
        break
      case 'm':
        for (let i = 0; i <= 21; i++) {
          res[i] = (11 - i === 0 ? '\\color{red}{' : '') + getDigitFromNumber(nbre, Decimal.pow(10, 11 - i).toNumber()) + (11 - i === 0 ? (new Decimal(nbre).decimalPlaces() === 0 ? '}' : ',}') : '')
        }
        break
      case 'dm':
        for (let i = 0; i <= 21; i++) {
          res[i] = (13 - i === 0 ? '\\color{red}{' : '') + getDigitFromNumber(nbre, Decimal.pow(10, 13 - i).toNumber()) + (13 - i === 0 ? (new Decimal(nbre).decimalPlaces() === 0 ? '}' : ',}') : '')
        }
        break
      case 'cm':
        for (let i = 0; i <= 21; i++) {
          res[i] = (15 - i === 0 ? '\\color{red}{' : '') + getDigitFromNumber(nbre, Decimal.pow(10, 15 - i).toNumber()) + (15 - i === 0 ? (new Decimal(nbre).decimalPlaces() === 0 ? '}' : ',}') : '')
        }
        break
      case 'mm':
        for (let i = 0; i <= 21; i++) {
          res[i] = (17 - i === 0 ? '\\color{red}{' : '') + getDigitFromNumber(nbre, Decimal.pow(10, 17 - i).toNumber()) + (17 - i === 0 ? (new Decimal(nbre).decimalPlaces() === 0 ? '}' : ',}') : '')
        }
        break
    }
    return res
  ...
  const createTab = function (aT: string[], rT:string[], first:number, end:number, ligne: number, correction = false, hectare = false) {
