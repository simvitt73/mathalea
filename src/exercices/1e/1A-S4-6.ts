...
${texteGras('série A :')} $~~${a1}~~; ~~${a2}~~; ~~${a3}~~$<br>
${texteGras('série B :')} $~~${texNombre(b1)}~~; ~~${texNombre(b2)}~~; ~~${texNombre(b3)}~~$<br>
...
Laquelle des ces 4 propositions est vraie ?`
    ...
    if (alea === 3) { //  moyennes et médianes différentes
      b2 = randint(Math.floor(b1 + 1), Math.floor(b1 + 3), a2)
...
Laquelle des ces 4 propositions est vraie ?`
    ...
    $\\overline{x}_A=\\dfrac{${a1} + ${a2} + ${a3}}{3} = ${moyA.texFraction}$`
    ...
    $\\overline{x}_B=\\dfrac{${b1} + ${b2} + ${b3}}{3} = ${moyB.texFraction} $`
    if (Number.isInteger(moyenneA)) {
      this.correction += ` $= ${moyB.texFractionSimplifiee}$<br>`
    }
    this.correction += `<br>et sa médiane, qui est aussi la valeur centrale de la série classée : $\\mathrm{m_B}=${medianeB}$ .<br>`
    ...
    if (alea === 3) { this.correction += '<br>On constate que les deux séries ont la même moyenne mais pas la même médiane ' }
    if (alea === 4) { this.correction += '<br>On constate que les deux séries n\'ont ni la même moyenne ni la même médiane ' }
  }
