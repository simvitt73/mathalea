...
export default class MoyenneClasseQCM extends ExerciceQcmA {
  ...
  void {
    ...
    if (effectifs[0] === effectifs[1]) { effectifs[1] = randint(3, 7, effectifs[0]) }
    // Génère les bornes des classes sous forme de chaînes '[a ; b['
    const classes = [`[${valeurInferieure} ; ${valeurInferieure + pas}[`, `[${valeurInferieure + pas} ; ${valeurInferieure + (2 * pas)}[`]
    ...
    const centres = [valeurInferieure + pas / 2, valeurInferieure + 3 * pas / 2]
    ...
    // ⚠️ Test de sécurité
    if (!Number.isInteger(moyenne) || moyenne < 1 || moyenne > 10) return this.appliquerLesValeurs()
    ...
    const tableau = tableauColonneLigne(['\\text{Taille}', ...classes], ['\\text{Effectifs}'], [...effectifs])
    const distracteur1 = Number(effectifs.reduce((a, b) => a + b, 0) / effectifs.length)
