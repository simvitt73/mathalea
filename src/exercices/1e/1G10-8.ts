...
export default class UtiliserProduitScalaire extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, texte, texteCorr, objets, nomA, nomB, nomC, nomD, nomE, nomF, AE, AF, BE, CF, listeLong, long, l1, l2, lE, lF, xA, xB, xC, xD, dx, dy, yA, yB, yC, yD, A, B, C, D, E, F, poly, seg1, seg2, indiceFirstLetter, listeValeurs, val, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      listeValeurs = [[3, 4, 2, 6], [-3, 4, 2, -6], [2, 6, 12, 1], [2, 9, 3, 6], [2, 3, 1, 6], [2, -9, 3, -6], [6, 5, 10, 3], [6, -5, -10, 3], [-5, 6, -3, 10], [2, -12, -6, 4], [3, -8, -6, 4], [2, 12, 1, 24], [2, -12, -8, 3],
        [3, 3, 1, 9], [3, -3, 1, -9], [3, 3, 1, 9], [2, 8, 4, 4], [-4, 4, -8, 2], [7, 4, 14, 2], [-14, 2, -7, 4]]//
       ...
       +${ecritureParentheseSiNegatif(yB - yA)}\\times${ecritureParentheseSiNegatif(yD - yC)}
        ...
        case 3 :
          switch (randint(1, 2)) {
            case 1 :
              objets = []
              listeLong = [[12, 4, 1, 1], [6, 4, 1, 2], [15, 5, 2, 1], [6, 3, 1, 1], [8, 4, 2, 1], [12, 6, 2, 2], [14, 7, 3, 2]]// AB, AD, AE, AF,
              ...
              lE = latex2d(`${nomE}`, -0.5, long[2], { letterSize: 'normalsize' })
              lF = latex2d(`${nomF}`, long[3], -0.5, { letterSize: 'normalsize' })
              poly = polygoneAvecNom(A, B, C, D)
              objets.push(poly, seg1, seg2, lE, lF, l1, l2)
              texte = `Dans un rectangle $${nomA}${nomB}${nomC}${nomD}$ de longueur $${long[0]}$ et de largeur $${long[1]}$, on considère les points $${nomE}$ et $${nomF}$ tels que :<br>
              ...
              Montrer que les droites $(${nomE}${nomC})$ et $(${nomD}${nomF})$ sont perpendiculaires.<br>` + mathalea2d(Object.assign({ scale: 0.7, pixelsParCm: 25, style: 'inline' }, fixeBordures(objets)), objets)
              texteCorr = `D'une part, $\\overrightarrow{${nomE}${nomC}}= \\overrightarrow{${nomE}${nomD}}+ \\overrightarrow{${nomD}${nomC}}$.<br>
            ...
            default:
              objets = []
              listeLong = [[9, 6, 3, 2], [12, 4, 3, 1], [8, 4, 2, 1], [10, 5, 4, 2], [6, 4, 3, 2], [12, 8, 6, 4], [9, 6, 3, 2]]// AB, AD, BE, CF,
              long = choice(listeLong)
              yC = long[1]
              xB = long[0]
              ...
              lE = latex2d(`${nomE}`, long[0] + 0.5, long[2], { letterSize: 'normalsize' })
              lF = latex2d(`${nomF}`, long[0] - long[3], long[1] + 0.5, { letterSize: 'normalsize' })
              poly = polygoneAvecNom(A, B, C, D)
              objets.push(poly, seg1, seg2, lE, lF, l1, l2)
              texte = `Dans un rectangle $${nomA}${nomB}${nomC}${nomD}$ de longueur $${long[0]}$ et de largeur $${long[1]}$, on considère les points $${nomE}$ et $${nomF}$ tels que :<br>
             $\\overrightarrow{${nomB}${nomE}}=${BE.texFraction}\\overrightarrow{${nomB}${nomC}}$ et  $\\overrightarrow{${nomC}${nomF}}=${CF.texFraction}\\overrightarrow{${nomC}${nomD}}$.<br>
              Montrer que les droites $(${nomA}${nomE})$ et $(${nomB}${nomF})$ sont perpendiculaires.<br>` + mathalea2d(Object.assign({ scale: 0.7, pixelsParCm: 25, style: 'inline' }, fixeBordures(objets)), objets)
              texteCorr = `D'une part, $\\overrightarrow{${nomA}${nomE}}= \\overrightarrow{${nomA}${nomB}}+ \\overrightarrow{${nomB}${nomE}}$.<br>
            ...
            case 1 :
              ...
              texte = `Dans un rectangle $${nomA}${nomB}${nomC}${nomD}$ de longueur $${long[0]}$ et de largeur $${long[1]}$, on considère les points $${nomE}$ et $${nomF}$ tels que :<br>
               ...
               ` + createList(
                ...
                ) + mathalea2d(Object.assign({ scale: 0.7, pixelsParCm: 25, style: 'inline' }, fixeBordures(objets)), objets)
