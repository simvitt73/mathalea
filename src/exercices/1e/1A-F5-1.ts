...
export default class Auto1AF5a extends ExerciceQcmA {
  versionOriginale: () => void = () => {
    const f = (x:number | FractionEtendue) => (3 * Number(x) - 15) * (Number(x) + 2)
    const f1 = (x:number | FractionEtendue) => (3 * Number(x) - 15) * (Number(x) + 2) * (-1)
    const f2 = (x:number | FractionEtendue) => (3 * Number(x) + 15) * (Number(x) + 2)
    const f3 = (x:number | FractionEtendue) => (3 * Number(x) + 15) * (Number(x) - 2)
    ...
    this.enonce = 'La fonction $f$ définie sur $\\mathbb{R}$ par $f(x)=(3x-15)(x+2)$ admet pour tableau de signes :   '
    this.reponses = [
    ...
    this.correction = `L'équation $3x-15=0$ a pour solution $x=5$.<br>
    L'équation $x+2=0$ a pour solution $x=2$.<br>
    Le tableau de signe du produit $(3x+15)(x+2)$ est : <br>` + tableauDeVariation({
        tabInit: [
        // @ts-expect-error TableauDeVariation n'est pas typé correctement
          [
            ['$x$', 2, 30], ['$3x-15$', 2, 50], ['$x+2$', 2, 50], ['$(3x-15)(x+2)$', 2, 100]
          ],
  ...
  versionAleatoire: () => void = () => {
    ...
    const f = (x: number | FractionEtendue) => (a * Number(x) + b) * (m * Number(x) + p)
    const f1 = (x: number | FractionEtendue) => (a * Number(x) + b) * (m * Number(x) + p) * (-1)
    const f2 = (x: number | FractionEtendue) => (a * Number(x) - b) * (m * Number(x) + p)
    const f3 = (x: number | FractionEtendue) => (a * Number(x) - b) * (m * Number(x) - p)
    ...
    const ligneMMP = ['Line', 30, '', 0, '-', 20, 't', 5, '-', 20, 'z', 20, '+', 20]
    const lignePMP = ['Line', 30, '', 0, '+', 20, 'z', 20, '-', 20, 'z', 5, '+', 20]
    const ligneMPM = ['Line', 30, '', 0, '-', 20, 'z', 5, '+', 20, 'z', 20, '-', 20]
   ...
   `${tableauSignesFonction(f3,
    ...
    this.correction = `L'équation $${reduireAxPlusB(a, b)}=0$ a pour solution $x=${texNombre(-b / a)}$.<br>
  ...
  tableauDeVariation({
    tabInit: [
      ...
      ['$-\\infty$', 30, `$${texNombre(rMin)}$`, 20, `$${texNombre(rMax)}$`, 20, '$+\\infty$', 30]
    ],
    tabLines: [ligne1, ligne2, ligne3],
    colorBackground: '',
    espcl: 3,
