...
export default class CalculerProbaTableau extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, texte, texteCorr, reponse, cpt = 0, pA, pB, nA, nB, Total, calcul, calculCorr, tableau, pBb, nAinterB, pAsachantBb, pBsachantA, pAbinterBb, pAbinterB, pAinterBb, pAinterB, pBsachantAb, pAb, pBbsachantA, listeEV, ev; i < this.nbQuestions && cpt < 50;) {
      listeEV = [['A', 'B'], ['A', 'C'], ['R', 'T'], ['K', 'L'], ['R', 'U'], ['S', 'T']]
      ev = choice(listeEV)
      nA = randint(27, 80)
      nB = randint(41, 70)
      ...
      tableau = tableauColonneLigne(['', `${ev[0]}`, `\\overline{${ev[0]}}`, '\\text{Total}'],
      ...
      switch (randint(1, 4)) {
        case 1 :// PA, PAinterBb, PBsachantAb
          ...
          texte += ajouteChampTexteMathLive(this, 3 * i, KeyboardType.clavierDeBaseAvecFraction, { texteAvant: `<br>$${calcul[0]}=$` }) +
          ajouteChampTexteMathLive(this, 3 * i + 1, KeyboardType.clavierDeBaseAvecFraction, { texteAvant: `<br>$${calcul[1]}=$` }) +
          ajouteChampTexteMathLive(this, 3 * i + 2, KeyboardType.clavierDeBaseAvecFraction, { texteAvant: `<br>$${calcul[2]}=$` })
