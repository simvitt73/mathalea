...
export default class Auto1AF5a extends ExerciceQcmA {
  versionOriginale: () => void = () => {
    ...
    const fProduit = (x: number | FractionEtendue) => (a * Number(x) + b) * (m * Number(x) + p)
    const fDist1 = (x: number | FractionEtendue) => (a * Number(x) - b) * (m * Number(x) - p) * (-1)
    const fDist2 = (x: number | FractionEtendue) => (a * Number(x) + b) * (m * Number(x) + p) * (-1)
    const fDist3 = (x: number | FractionEtendue) => (a * Number(x) - b) * (m * Number(x) - p)
    ...
    const objet = mathalea2d({ xmin, xmax, ymin: ymin - 0.25, ymax: ymax + 0.25, pixelsParCm: 30, scale: 0.6, style: 'margin: auto' }, courbe(f1, {
      repere: r,
      color: 'blue',
      epaisseur: 2
    }), courbe(f2, {
   ...
   `${tableauSignesFonction(fDist3,
    ...
    this.correction = `La fonction $g$ s'annule en $x = -3$ et la fonction $h$ s'annule en $x = 1$.<br>
  ...
  versionAleatoire: () => void = () => {
   ...
   `${tableauSignesFonction(fDist3,
    ...
    this.correction = `La fonction $g$ s'annule en $${texNombre(racine1)}$ et la fonction $h$ s'annule en $${texNombre(racine2)}$.<br>
    Quand la droite est en-dessous de l'axe des abscisses, la fonction est négative et quand elle est au-dessus, la fonction est positive.<br>
    On en déduit le tableau de signes de leur produit :` +
  tableauDeVariation({
    tabInit: [
