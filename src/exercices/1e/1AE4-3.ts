...
export default class EvolSuccEffet extends ExerciceQcmA {
  ...
  versionOriginale: () => void = () => {
   ...
   ${texteEnCouleurEtGras('Le prix final sera donc strictement inférieur au prix initial.')}`
    this.reponses = ['Le prix est strictement inférieur à sa valeur de départ. ', 'Le prix est égal à sa valeur de départ. ',
      'Le prix est strictement supérieur à sa valeur de départ. ', ' On ne peut pas savoir : cela dépend de la valeur de départ.  ']
  }

  versionAleatoire: () => void = () => {
    switch (choice([1, 2, 2, 2, 3, 3, 3])) { //
      case 1: {
...
Sachant que le taux réciproque d'une baisse de $20\\,\\%$ est une augmentation de $25\\,\\%$, après ces deux évolutions, on peut affirmer que :  `
    ...
    ${texteEnCouleurEtGras('Le prix final sera donc strictement supérieur au prix initial.')}`
            this.reponses = ['Le prix est strictement supérieur à sa valeur de départ. ', 'Le prix est égal à sa valeur de départ. ',
              'Le prix est strictement inférieur à sa valeur de départ. ', ' On ne peut pas savoir : cela dépend de la valeur de départ.  ']
          } else
            if (taux === 25) {
              this.correction = `
      Le taux réciproque d'une baisse de $20\\,\\%$ est une augmentation de $25\\,\\%$.<br>
      ...
      ${texteEnCouleurEtGras('Le prix final sera donc égal au prix initial.')}`
              this.reponses = ['Le prix est égal à sa valeur de départ. ', 'Le prix est strictement inférieur à sa valeur de départ. ',
                'Le prix est strictement supérieur à sa valeur de départ. ', ' On ne peut pas savoir : cela dépend de la valeur de départ.  ']
            } else {
              this.correction = `
      Le taux réciproque d'une baisse de $20\\,\\%$ est une augmentation de $25\\,\\%$.<br>
