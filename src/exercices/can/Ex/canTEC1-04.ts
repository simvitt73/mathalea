...
this.correction =
        `Par définition, on sait que $\\mathrm{i}^{2}=-1$ `
    ...
    case 2:
      ...
      this.correction =
        `On sait que $\\left(\\mathrm{-i}\\right)^{${k}}=\\left(\\mathrm{-1}\\right)^{${k}}\\times \\mathrm{i}^{${k}}$. <br>`
     this.correction +=
        'Par définition, on a $\\mathrm{i}^{2}=-1$ '

    if (k%4===0) {
      if (k===4) {this.correction += ` donc $\\mathrm{i}^{4}=1$ et finalement $\\left(\\mathrm{-i}\\right)^{4}=${miseEnEvidence('1')}$ .`}
      else {
        this.correction +=
        ` donc $\\mathrm{i}^{4}=1$ .<br>
     ...
     if (k===3) {
      this.correction +=
         `  <br> 
         $\\begin{aligned}
         \\left(\\mathrm{-i}\\right)^{3}&= -\\mathrm{i}^{3}\\\\
         &=-\\mathrm{i}^{2}\\times \\mathrm{i}\\\\
         &=${miseEnEvidence('\\mathrm{i}.')}
         \\end{aligned}$`
      this.reponses = [
