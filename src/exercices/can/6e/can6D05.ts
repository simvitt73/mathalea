...
export const titre = 'Connaître les unités de mesure de durées courtes et les relations qui les lient'
...
export default class AutoDureeMinutes extends ExerciceSimple {
  ...
  nouvelleVersion () {
    const durees = [['jour', 'heures', 24], ['heure', 'minutes', 60], ['minute', 'secondes', 60]]
    const choix = randint(0, 2)

    this.reponse = durees[choix][2]
    this.question = `$1$ ${durees[choix][0]} = `
    this.canReponseACompleter = this.question + '$\\ldots\\ldots$ ' + durees[choix][1]
