...
export default class CalculerAngle extends ExerciceSimple {
  ...
  const codage=codageSegments('//', 'black', A, C, B, C)
    ...
    const angleExt = afficheMesureAngle(C, B, E, 'black', 0.8, `${angleExterieur}°`)
    
    // Affichage de x au sommet (angle C)
    const angleC = afficheMesureAngle(A, C, B, 'black', 0.8, 'x')

    const objets = [segmentHorizontal, coteAC, coteBC, labels, angleExt, angleC, codage]
    ...
    this.reponse = angleX
        ...
        this.question = 
          `Déterminer la valeur de $x$.<br>`+figure 
    ...
    default:
     {
        ...
        const angleExt = afficheMesureAngle(B, C, E, 'black', 0.8, `${angleExterieur}°`)
        
        // Affichage de x en A (angle à la base gauche)
        const angleA = afficheMesureAngle(C, A, B, 'black', 0.8, 'x')

        const objets = [segmentHorizontal, coteAC, coteAB, coteBC, labels, angleExt, angleA, codage]
