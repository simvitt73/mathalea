...
export const titre = 'Déterminer le domaine de définition d\'une fonction inverse ou racine'
...
export default class DomaineDeDefinition extends Exercice {
  constructor () {
    ...
    this.consigne = ''
    this.nbQuestions = 1
    this.besoinFormulaireNumerique = ['Type de questions', 8, '1 : 1/q(x)\n2 : p(x)/q(x)\n3 : √p(x)\n4 : √p(x)/q(x)\n5 : p(x)/√q(x)\n6 : √p(x)/√q(x)\n7 : √p(x)+√q(x)\n8 : Mélange']
    this.besoinFormulaire2Numerique = ['Degrés des polynômes', 3, '1 : 1er degré\n2 : 2e degré\n3 : Mélange']
    this.sup2 = 3
    this.sup = 1
  }
  ...
  solutionEq (p: PolynomePlusieursVariables) : FractionEtendue {
    if (p.monomes.length === 1) {
      ...
      const termConst = p.monomes.find((monome) => monome.degre === 0) as MonomePlusieursVariables
      const termLin = p.monomes.find((monome) => monome.degre === 1) as MonomePlusieursVariables
      return termConst.coefficient.oppose().diviseFraction(termLin.coefficient).simplifie()
    }
  }
  ...
  computeDomainePolynome (type : string, p : PolynomePlusieursVariables, racinesP : FractionEtendue[]) : string[][] {
    if (type === 'sqrt') {
      if (racinesP.length === 1) {
        if (p.monomes.find((monome) => monome.degre === 1)?.coefficient.signe === 1) {
        ...
        if (p.monomes.find((monome) => monome.degre === 2)?.coefficient.signe === 1) {
          return [['o', '-infty', 'f', racinesP[0].texFractionSimplifiee], ['f', racinesP[1].texFractionSimplifiee, 'o', '+infty']]
        } else if (p.monomes.find((monome) => monome.degre === 2)?.coefficient.signe === -1) {
          return [['f', racinesP[0].texFractionSimplifiee, 'f', racinesP[1].texFractionSimplifiee]]
        }
      }
      ...
      if (racinesP.length === 1) {
        ...
        if (p.monomes.find((monome) => monome.degre === 2)?.coefficient.signe === 1) {
          return [['o', '-infty', 'o', racinesP[0].texFractionSimplifiee], ['o', racinesP[1].texFractionSimplifiee, 'o', '+infty']]
        } else if (p.monomes.find((monome) => monome.degre === 2)?.coefficient.signe === -1) {
          return [['o', racinesP[0].texFractionSimplifiee, 'o', racinesP[1].texFractionSimplifiee]]
        }
      }
      ...
      if (racinesP.length === 1) {
        ...
        return [['R', racinesP[0].texFractionSimplifiee, racinesP[1].texFractionSimplifiee]]
      }
    }
    return [['']]
  }
  ...
  // Both are R–type:
    ...
    if (
      lowerVal > upperVal ||
    (lowerVal === upperVal && (newLowerMarker === 'o' || newUpperMarker === 'o'))
    ) {
    ...
    exclusions.forEach(ex => {
      ...
      intervals.forEach(inter => {
        ...
        if (exVal <= lVal || exVal >= uVal) {
          ...
          if (this.parseEndpoint(left[1]) < this.parseEndpoint(left[3])) newIntervals.push(left)
          if (this.parseEndpoint(right[1]) < this.parseEndpoint(right[3])) newIntervals.push(right)
        }
      })
  ...
  for (const cur of objs) {
    if (merged.length === 0) {
      ...
      if (cur.left < last.right || (cur.left === last.right && (last.rightMarker === 'f' || cur.leftMarker === 'f'))) {
  ...
  if (merged.length === 1 && merged[0].left === -Infinity && merged[0].right === Infinity) {
    return [['R']]
  }
  if (merged.length === 0) return [[]] // no intervals left
  // Convert merged objects back to our string[][] format.
  return merged.map(obj => [obj.leftMarker, obj.leftStr, obj.rightMarker, obj.rightStr])
  }
  ...
  intersectDomains (domain1: string[][], domain2: string[][]): string[][] {
    ...
    const allSqrt = result.every(interval => interval.length === 4 && interval[0] !== 'R')
    if (allSqrt) {
  ...
  convertDomainToLatex (domain: string[][]): string {
    ...
    for (const interval of domain) {
      ...
      if (interval.length === 0) {
        ...
        const exclusions = interval.slice(1).map(convertEndpoint).join('\\,;\\,')
        intervalsLatex.push(`\\mathbb{R}\\setminus\\left\\{${exclusions}\\right\\}`)
        ...
        intervalsLatex.push(`${leftBracket}${leftValLatex}\\,;\\,${rightValLatex}${rightBracket}`)
      } else {
  ...
  nouvelleVersion () {
    ...
    const typesDeQuestionsDisponibles = gestionnaireFormulaireTexte({
      ...
      melange: 8,
      defaut: 1,
      listeOfCase: ['1/q', 'p/q', 'racineP', 'racineP/q', 'p/racineQ', 'racineP/racineQ', 'racineP+racineQ'],
    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      let texte = ''
      let texteCorr = ''
      texte = 'Déterminer le domaine de définition de la fonction $f$ définie par $f(x) = '
      ...
      if (degP === 2 && degQ === 2) {
        do {
          const a = new FractionEtendue(randint(-10, 10, [0]), randint(-10, 10, [0])).simplifie()
          const b = new FractionEtendue(randint(-10, 10, [0]), randint(-10, 10, [0])).simplifie()
          const c = new FractionEtendue(randint(-10, 10, [0]), randint(-10, 10, [0])).simplifie()
          const d = new FractionEtendue(randint(-10, 10, [0]), randint(-10, 10, [0])).simplifie()
          pEq = EquationSecondDegre.aPartirDesSolutions(a, b, new FractionEtendue(a.den * b.den, 1), { variable: 'x', format: 'reduit' })
          qEq = EquationSecondDegre.aPartirDesSolutions(c, d, new FractionEtendue(c.den * d.den, 1), { variable: 'x', format: 'reduit' })
        } while (pEq.equationTex === qEq.equationTex && pEq.delta.signe !== 1 && qEq.delta.signe !== 1)
        ...
        p = pEq.polynomeFormeReduite()
        racinesP = pEq.solutionFrac() as FractionEtendue[]
        q = PolynomePlusieursVariables.createRandomPolynome(1, 1, 2, 'entier', ['x'])
        racinesQ = [this.solutionEq(q)]
      } else if (degP === 2 && degQ === 1) {
        do {
          const a = new FractionEtendue(randint(-10, 10, [0]), randint(-10, 10, [0])).simplifie()
          const b = new FractionEtendue(randint(-10, 10, [0]), randint(-10, 10, [0])).simplifie()
          qEq = EquationSecondDegre.aPartirDesSolutions(a, b, new FractionEtendue(a.den * b.den, 1), { variable: 'x', format: 'reduit' })
        } while (qEq.delta.signe !== 1)
        q = qEq.polynomeFormeReduite()
        racinesQ = qEq.solutionFrac() as FractionEtendue[]
        p = PolynomePlusieursVariables.createRandomPolynome(1, 1, 2, 'entier', ['x'])
