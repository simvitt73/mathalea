...
export default class CalculProbaTab2Entrées extends Exercice {
  ...
  nouvelleVersion () {
    const typeDeQuestion = combinaisonListes([entreprise, voliere, bouquet, panierDeFruits, voitures, scolaires, aliments, appareils], this.nbQuestions)
...
function entreprise (exercice: Exercice, question: number): { texte: string, texteCorr: string, alea: (string | number)[] } {
  ...
  const nbPart2B = randint(5 * tailleEntreprise, 8 * tailleEntreprise, [nbPart1B, nbPart2A])
  const lots = [
  ...
  return moule(exercice, question, 'femme(f)', 'homme', 'cadre', 'non cadre', 'employé', `Le comité d'une entreprise décide de faire un tirage au sort pour choisir un salarié qui gagnera ${choice(lots)}.<br>`, 'le gagnant du tirage au sort', [nbPart1A, nbPart2A, nbPart1B, nbPart2B])
}
// On capture au hasard un volatile dans une volière
function voliere (exercice: Exercice, question: number): { texte: string, texteCorr: string, alea: (string | number)[] } {
  ...
  return moule(exercice, question, 'perruche(f)', 'perroquet', 'femelle(f)', 'mâle', 'volatile', 'Dans une volière, il y a des perroquets et des perruches. On capture au hasard un volatile.<br>', 'le volatile capturé', [nbPart1A, nbPart2A, nbPart1B, nbPart2B])
}

// On choisit au hasard une fleur dans un bouquet
function bouquet (exercice: Exercice, question: number): { texte: string, texteCorr: string, alea: (string | number)[] } {
  ...
  return moule(exercice, question, 'rose(f)', 'tulipe(f)', 'rouge(f)', 'jaune(f)', 'fleur', 'On choisit au hasard une fleur dans un bouquet de roses et de tulipes.<br>', 'la fleur choisie', [nbPart1A, nbPart2A, nbPart1B, nbPart2B])
}

// On choisit au hasard un fruit dans un panier
function panierDeFruits (exercice: Exercice, question: number): { texte: string, texteCorr: string, alea: (string | number)[] } {
...
function voitures (exercice: Exercice, question: number): { texte: string, texteCorr: string, alea: (string | number)[] } {
  const nbPart1A = randint(3, 7)
  const nbPart2A = randint(3, 7, nbPart1A)
  const nbPart1B = randint(5, 10, nbPart1A)
  const nbPart2B = randint(5, 10, [nbPart1B, nbPart2A])
...
function scolaires (exercice: Exercice, question: number): { texte: string, texteCorr: string, alea: (string | number)[] } {
...
function aliments (exercice: Exercice, question: number): { texte: string, texteCorr: string, alea: (string | number)[] } {
  const nbPart1A = randint(3, 7)
  const nbPart2A = randint(3, 7, nbPart1A)
  const nbPart1B = randint(5, 10, nbPart1A)
  const nbPart2B = randint(5, 10, [nbPart1B, nbPart2A])
...
function appareils (exercice: Exercice, question: number): { texte: string, texteCorr: string, alea: (string | number)[] } {
  const nbPart1A = randint(3, 7)
  const nbPart2A = randint(3, 7, nbPart1A)
  const nbPart1B = randint(5, 10, nbPart1A)
  const nbPart2B = randint(5, 10, [nbPart1B, nbPart2A])
...
function moule (exercice: Exercice, question: number, subst1: string, subst2: string, adj1: string, adj2: string, substT: string, intro: string, choix: string, nbParts:[number, number, number, number]): { texte: string, texteCorr: string, alea: number[] } {
  ...
  const ligneEnt = [`\\text{${premiereLettreEnMajuscule(adj1)}s}`, `\\text{${premiereLettreEnMajuscule(adj2)}s}`, '\\text{Total}']
  const colonneEnt = ['\\text{~}', `\\text{${premiereLettreEnMajuscule(subst1)}s}`, `\\text{${premiereLettreEnMajuscule(subst2)}s}`, '\\text{Total}']
  const contenu = []
  ...
  const contenuCorr = [
    nbPart1A, nbPart2A, nbTotalA,
    nbPart1B, nbPart2B, nbTotalB,
    nbTotal1, nbTotal2, nbTotal
  ]
  ...
  const tableauInteractif = AddTabDbleEntryMathlive.create(Number(exercice.numeroExercice), question * 4, AddTabDbleEntryMathlive.convertTclToTableauMathlive(colonneEnt, ligneEnt, contenu), '', true, {})
  question1 += exercice.interactif ? tableauInteractif.output : tableau
  handleAnswers(exercice, question * 4, objetReponse1, { formatInteractif: 'mathlive' })
  ...
  const question2 = `Quelle est la probabilité que ${choix} soit un${Fem1} ${subst1} ${adj1} ou un${Fem2} ${subst2} ${adj1} ?` + ajouteQuestionMathlive({
    exercice,
    question: question * 4 + 1,
    objetReponse: { reponse: { value: proba2.texFraction, options: { fractionEgale: true } } },
  ...
  const question3 = `Quelle est la probabilité que ${choix} soit un${Fem2} ${subst2} ?` + ajouteQuestionMathlive({
    exercice,
    question: question * 4 + 2,
    objetReponse: { reponse: { value: proba3.texFraction, options: { fractionEgale: true } } },
    typeInteractivite: 'mathlive'
  })
  ...
  const question4 = `On sait que ${choix} est un${Fem1} ${subst1}. Quelle est la probabilité que ce soit un${Fem1} ${subst1} ${adj1} ?` + ajouteQuestionMathlive({
    exercice,
    question: question * 4 + 3,
    objetReponse: { reponse: { value: proba4.texFraction, options: { fractionEgale: true } } },
    typeInteractivite: 'mathlive'
  })
  ...
  const texteCorr = createList({
    ...
    style: 'nombres'
  }).replaceAll('fruits vertes', 'fruits verts').replaceAll('Androïds', 'Androïd').replaceAll('IOSs', 'iOS') // Pas trouvé comment régler ça dans le code
