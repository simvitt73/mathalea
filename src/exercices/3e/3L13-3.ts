...
function figureThales (a: number | string, b: number | string, c: number | string, OC: number | string): string {
...
function basket (cd: boolean) {
  ...
  const resolution = resoudre(equation, { reduceSteps: false, substeps: true, comment: cd })
  ...
  return { enonce, intro, conclusion, figure, verification, uniteOptions, x, resolution }
}
...
function basket2 (cd: boolean) {
  ...
  const resolution = resoudre(equation, { reduceSteps: false, substeps: true, comment: cd, suppr1: false })
  let enonce = `Une équipe de basket a marqué ${d} points lors d'un match. Au cours de ce match, elle a marqué ${b} points sur lancers francs.<br>`
  ...
  const uniteOptions = ['', '', '']
  const verification = `<br>Vérification :<br>$${resolution.verifLeftSide!.printExpression}=${resolution.verifLeftSide!.printResult}$`
  return { enonce, intro, conclusion, figure, uniteOptions, verification, x, resolution }
}
...
function polyg (valeurEntiere: boolean, cd: boolean) {
  ...
  const resolution = resoudre(equation, { reduceSteps: true, substeps: false, comment: cd })
...
function programme1 (n: 1 | 2, cd: boolean) {
  ...
  const resolution = resoudre(equation, { reduceSteps: false, substeps: false, comment: cd })
...
function thales (cd: boolean) {
  ...
  let enonce = 'Soit la figure ci-dessous qui n\'est pas en vraie grandeur où $[CD]$ et $[AB]$ sont parallèles.'
  enonce += ` $AB=${c}\\text{mm}$, $AC=${b}\\text{mm}$ et $CD=${a}\\text{mm}$.<br> Déterminer la longueur $OC$${context.isAmc ? ', en mm.' : '.'}`
  let intro = 'Dans cette configuration de Thalès, on a l\'égalité suivante : $\\dfrac{OC}{OA}=\\dfrac{CD}{AB}$.<br>'
...
function thales2 (cd: boolean) {
  ...
  let intro = 'Dans cette configuration de Thalès, on a l\'égalité suivante : $\\dfrac{OA}{OC}=\\dfrac{AB}{CD}$.<br>'
...
const listeDeFonction = [basket, basket2, [aliasAchatsEntier, aliasAchatsReel], [aliasPolygoneEntier, aliasPolygoneReel], [aliasProgramme1a, aliasProgramme1b], [aliasProgramme2a, aliasProgramme2b], [aliasTarifsEntier, aliasTarifsReel], [aliasSpectacleEntier, aliasSpectacleReel], isocele, thales, thales2]
...
export default class ProblemesEnEquation extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      if (Array.isArray(fonctionProbleme)) fonctionProbleme = fonctionProbleme[this.sup2 ? 0 : 1]
      let { enonce, intro, conclusion, figure, verification, uniteOptions, x, resolution } = fonctionProbleme(this.correctionDetaillee)
      figure = figure ?? ''
      uniteOptions = uniteOptions ?? ['', '', '']

      const texte = enonce + figure + ajouteChampTexteMathLive(this, i, '' + uniteOptions[0], { texteApres: sp(2) + uniteOptions[2] })
      let texteCorr = intro
      ...
      if (this.questionJamaisPosee(i, x, resolution.texteCorr)) {
        ...
        if (uniteOptions[0] === '') handleAnswers(this, i, { reponse: { value: x } })
        else handleAnswers(this, i, { reponse: { value: uniteOptions[1], options: { unite: true } } })
        i++
      }
      cpt++
    }
