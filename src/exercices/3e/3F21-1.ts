...
export default class LectureExpressionFonctionsAffines extends Exercice {
  ...
  nouvelleVersion (numeroExercice:number) {
    ...
    if (!this.lineaire) {
      ...
      this.contenuCorrection = 'Il s\'agit de fonctions linéaires, elles sont donc de la forme $f(x)=ax$, $a$ étant la ' + katexPopup2(1, 'pente', 'pente d\'une droite', 'La pente (le a de y=ax ou y=ax+b) d\'une droite donne le taux d\'accroissement de y par rapport à x : lorsque x augmente de 1, alors y augmente de a.') + ' de la droite.\n' + this.contenuCorrection
    }
    if (context.isAmc) {
