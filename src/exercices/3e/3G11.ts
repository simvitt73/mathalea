...
export default class ConstruireHomothetiePoint3e extends Exercice {
  ...
  nouvelleVersion (numeroExercice: number) {
    ...
    for (let i = 0, s, cpt = 0, texte, texteCorr; i < this.nbQuestions && cpt < 100;) {
      let borduresEnonce; let borduresCorrection; let largeurMax; let hauteurMax
      do {
        ...
        largeurMax = Math.max(borduresEnonce.xmax - borduresEnonce.xmin, borduresCorrection.xmax - borduresCorrection.xmin)
        hauteurMax = Math.max(borduresEnonce.ymax - borduresEnonce.ymin, borduresCorrection.ymax - borduresCorrection.ymin)
        texte += '<br>' + mathalea2d(Object.assign({}, borduresEnonce, { pixelsParCm: 20, scale: 0.5 }), objetsEnonce)
        propositionsAMC[0].propositions[0].enonce = texte + '<br>' + propositionsAMC[0].propositions[0].enonce
        texteCorr += '<br>' + mathalea2d(Object.assign({}, borduresCorrection, { pixelsParCm: 20, scale: 0.5 }), objetsCorrection)
