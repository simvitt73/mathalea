...
function meilleurPrix (prixPotA: number, prixPotB: number, nbLitres: number): string {
  if (nbLitres <= 5) {
...
export default class ExerciceAmeriqueNord392024 extends ExerciceBrevetA {
  constructor () {
    ...
    this.sup = false
    this.introduction = texteItalique('D\'après l\'exercice 4 du brevet Amérique du Nord 2024.<br>')

    this.versionAleatoire()
  }

  private appliquerLesValeurs (nomFamille: string, hauteur: number, grandeBase: number, petiteBase: number, epaisseur: number, prixPotA: number, prixPotB: number) : void {
    ...
    const epP = placeLatexSurSegment(`${epaisseur}~\\mathrm{cm}`, point(13.7, 2.25), point(13.7, 0.75), { distance: 0.5, letterSize: 'small' })
    ...
    aretesPointillees.forEach((arete) => { arete.pointilles = 2 })
    const segmentsP = [
      segment(FP, JP),
      segment(AP, DP),
      segment(GP, FP),
    ...
    const objetsP = [
      ...aretesPointillees, ...segmentsP, poly1P, labelsP, flecheP, epP
    ]
    const figurePersp = mathalea2d(Object.assign({ pixelsParCm: 25, scale: 0.5 }, fixeBordures(objetsP)), objetsP)
    ...
    const pbD = placeLatexSurSegment(`${texNombre(petiteBase, 1)}~\\mathrm{m}`, HD, GD, {})
    const gbD = placeLatexSurSegment(`${texNombre(grandeBase, 1)}~\\mathrm{m}`, JD, ED, { distance: 1 })
    const hD = placeLatexSurSegment(`${texNombre(hauteur, 1)}~\\mathrm{m}`, ED, HD, { distance: 0.5 })
    const flecheD = segment(point(0, -1.5), point(14, -1.5))
    flecheD.styleExtremites = '<->'
    const objetsD = [
      poly1D, GFPointilles, labelsD, angDroitD, flecheD, pbD, gbD, hD
    ]

    const fj = grandeBase - petiteBase
    const gj = Math.sqrt(hauteur * hauteur + fj * fj)
    const perimetre = gj + grandeBase + petiteBase + hauteur
    const figureDessus = mathalea2d(Object.assign({ pixelsParCm: 25, scale: 0.5 }, fixeBordures(objetsD)), objetsD)
...
${Number.isInteger(perimetre)
      ...
      : `Il leur faudra au minimum $${texNombre(perimetre, 1)}~\\mathrm{m}$`} de longueur de planches, mais en général on prévoit plus à cause des découpes.`
...
-- quantité de sable nécessaire $${texNombre(masseCiment, 0)}\\times ${texNombre(2.5, 1)}= ${texNombre(masseSable, 1)}~\\mathrm{kg}$.`
    ...
    const question3c = 'Pour faire du béton, on ajoute de l\'eau à un mélange de ciment, de gravier et de sable. Dans ce mélange, les masses de ciment - gravier - sable sont dans le ratio $2~:~7~:~5$. Déterminer (en kg), la masse de gravier et la masse de sable nécessaires pour réaliser les $4~\\mathrm{m}^{3}$ de béton.'
    const question3 = createList({
      items: [question3a, question3b, question3c],
      style: 'alpha'
    })
    const entetesCols = ['', '\\text{Pot A}', '\\text{Pot B}']
    const entetesLignes = ['\\text{Contenance (en litres)}', '\\text{Prix (en euros)}']
...
${listeDocs}`
  ...
  versionOriginale: () => void = () => {
    this.appliquerLesValeurs('Martin', 3, 10, 6, 15, 79.90, 129.90)
  }

  versionAleatoire: () => void = () => {
    const nomFamille = choice(['Martin', 'Dupont', 'Leroy', 'Bernard', 'Moreau', 'Lemoine', 'Garnier', 'Roussel', 'Bourgeois'])
    let hauteur: number
    ...
    do {
      ...
      prixPotB = randint(Math.ceil(prixPotA * 15.5), Math.floor(prixPotA * 17)) / 10
      const surfaceTerrasse = (petiteBase + grandeBase) * hauteur / 2
      surfacePeinture = surfaceTerrasse * 2
    } while (surfacePeinture / 5 > 20)
    this.appliquerLesValeurs(nomFamille, hauteur, grandeBase, petiteBase, epaisseur, prixPotA, prixPotB)
  }
}
