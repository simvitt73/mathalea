...
export default class ExerciceCentresEtrangers262024 extends ExerciceBrevetA {
  ...
  ${createList({
    ...
    On a donc bien besoin de : $${nbEx1} \\times ${dureeEx1 + repos1} = ${duree1}$ secondes pour effectuer le circuit 1.<br>
    ...
    On a donc bien besoin de ${duree2} secondes pour effectuer le circuit 2.`
    const question2 = `Donner la décomposition en produit de facteurs premiers de ${duree1} et de ${duree2}.`
    const correction2 = `$${duree1}=${decompositionFacteursPremiers(duree1)}$ soit $${texFactorisation(duree1, true)}$<br>
    $${duree2}=${decompositionFacteursPremiers(duree2)}$ soit $${texFactorisation(duree2, true)}$`
    ...
    ${createList({
      items: [
        `Expliquer pourquoi, lorsque ${duree3} secondes se sont écoulées à partir du coup de sifflet, ${prenom1} se trouve de nouveau au départ du circuit 1.<br>
        Préciser où se trouve ${prenom2} sur le circuit 2 lorsque ${duree3} secondes se sont écoulées.`,
    ...
    const correction3aa = `Tout d'abord la division euclidienne de $${duree3}$ par $${duree2}$ donne cette égalité : $${duree3}=${duree2}\\times${quotient}+${reste}$.<br>
    ...
    Donc depuis le coup de sifflet, il s'est écoulé : $${duree3}=${duree2}\\times${quotient}+${dureeEx2PlusRep2}\\times${quotient2}+${reste2}$ secondes.<br>
    ${prenom2} a donc effectué $${quotient}$ fois le circuit 2 complètement et ${quotient2 === 0 ? 'n\'a pas encore terminé le premier exercice.' : `a fait ensuite $${quotient2}$ exercices complètement avec le repos associé.`}<br>
    ...
    const correction3a = `Lorsque ${duree3} secondes se sont écoulées à partir du coup de sifflet, ${prenom1} se trouve de nouveau au départ du circuit 1 car $${duree3} = ${duree1}\\times${duree3 / duree1}$, donc comme $${duree3 / duree1}$ est un nombre entier, cela signifie que ${prenom1} a effectué $${duree3 / duree1}$ fois le circuit 1 complètement, et n'a pas encore commencé la $${duree3 / duree1 + 1}$ième répétition : ${prenom1} est donc à nouveau au départ du circuit 1.<br>`
    ...
    En écrivant la liste des multiples de ces deux nombres, le premier multiple commun est $${ppcM}=60\\times${Math.floor(ppcM / 60)}${ppcM % 60 === 0 ? '' : `+${ppcM % 60}`}$.<br>
    $${ppcM}~\\text{s}=${duree.minute}~\\text{min}${ppcM % 60 === 0 ? '' : `~${duree.second}~\\text{s}`}$.<br>`
