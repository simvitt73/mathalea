...
export const titre = 'Préparation DNB : Probabilités, arithmétique et nombres premiers'
...
export default class ExercicePolynesie462024 extends ExerciceBrevetA {
  constructor () {
    ...
    this.sup = false
    this.introduction = texteItalique('D\'après l\'exercice 4 du brevet Polynésie Juin 2024.<br>')

    this.versionAleatoire()
  }

  private appliquerLesValeurs (u1B1: number, u1B2: number, u1B3: number, u2B1: number, u2B2: number, u3B1: number, u3B2: number, prenom1: string, prenom2: string, resultat2: number, facteur: number) : void {
    ...
    const objets = [urne1, urne2, b1U1, b2U1, b3U1, b1U2, b2U2, n1U1, n2U1, n3U1, n1U2, n2U2]
    const figure = mathalea2d(Object.assign({ pixelsParCm: 20, scale: 0.5, style: 'display: inline' }, fixeBordures(objets)), objets)

    this.enonce = `On dispose de deux boîtes contenant des boules numérotées, indiscernables au toucher.<br>
    ${deuxColonnesResp(` La première contient trois boules numérotées ${u1B1}, ${u1B2} et ${u1B3}.<br>
  La deuxième contient deux boules numérotées ${u2B1} et ${u2B2}.`, figure, { largeur1: 60, widthmincol1: '400px', widthmincol2: '200px' })}
    ...
    const entetesCols = ['_{\\text{boite 1}}\\backslash ^{\\text{boite 2}}', `~${u2B1}~`, `~${u2B2}~`]
    ...
    ${tableau}`
    const lignesCorr = [String(u1B1 * u2B1), String(u1B1 * u2B2), String(u1B2 * u2B1), String(u1B2 * u2B2), String(u1B3 * u2B1), String(u1B3 * u2B2)]
    const tableauCorr = tableauColonneLigne(entetesCols, entetesLignes, lignesCorr)
  ...
  versionAleatoire: () => void = () => {
    ...
    this.appliquerLesValeurs(u1B1, u1B2, u1B3, u2B1, u2B2, u3B1, u3B2, Array.isArray(prenom1) ? prenom1[0] : prenom1, Array.isArray(prenom2) ? prenom2[0] : prenom2, resultat2, facteur)
  }
}
