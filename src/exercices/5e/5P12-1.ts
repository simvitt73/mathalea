...
export default class ProblemeDeRatio extends Exercice {
  ...
  nouvelleVersion () {
    ...
    let article; let p1; let quidam; let index; let index2
    ...
    const typeDeProblèmes = gestionnaireFormulaireTexte({ saisie: this.sup2, nbQuestions: this.nbQuestions, min: 1, max: 5, melange: 6, defaut: 1, listeOfCase: ['partage', 'mélange', 'dilution', 'recette', 'ecran'] }).map(String)
    for (let i = 0, texte, texteCorr, x:number, y: number, z, total, a:number, b: number, c:number, n = 2, k, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      index = randint(0, 10)
      index2 = randint(0, 10)
      ...
      k = randint(2, 6)
      total = (x + y + z) * k
      switch (typeDeProblèmes[i]) { // Chaque question peut être d'un type différent, ici 4 cas sont prévus...
        case 'partage':
          prenoms = personnes(n)
          for (let j = 0; j < n - 1; j++) {
            if (j > 0) {
          ...
          if (n === 2) {
            ...
            if (this.interactif) {
              texte += ajouteQuestionMathlive({
                ...
                objetReponse: { champ1: { value: k * x }, champ2: { value: k * y }, bareme: toutAUnPoint },
            ...
            if (this.interactif) {
              texte += ajouteQuestionMathlive({
                ...
                objetReponse: { champ1: { value: k * x }, champ2: { value: k * y }, champ3: { value: k * z }, bareme: toutAUnPoint },
                typeInteractivite: 'remplisLesBlancs'
              })
            }
          }
            ...
            if (choice([true, false])) {
              ...
              if (this.interactif) {
                texte += ajouteQuestionMathlive({
                  exercice: this,
                  question: i,
                  content: '\\text{Sirop : }%{champ1}\\text{ cL}\\text{ ;  eau : }%{champ2}\\text{ cL}',
                  objetReponse: { champ1: { value: k * x }, champ2: { value: k * y }, bareme: toutAUnPoint },
                  typeInteractivite: 'remplisLesBlancs'
                })
              }
              ...
              if (this.interactif) {
                texte += ajouteQuestionMathlive({
                  exercice: this,
                  question: i,
                  content: '\\text{eau : }%{champ1}\\text{ cL}\\text{ ;  boisson : }%{champ2}\\text{ cL}',
                  objetReponse: { champ1: { value: k * y }, champ2: { value: k * y + k * x }, bareme: toutAUnPoint },
                  typeInteractivite: 'remplisLesBlancs'
                })
              }
            }
            ...
            if (choice([true, false])) {
              ...
              if (this.interactif) {
                texte += ajouteQuestionMathlive({
                  ...
                  objetReponse: { champ1: { value: String(k * x) }, champ2: { value: String(k * y) }, champ3: { value: String(k * z) }, bareme: toutAUnPoint },
                  typeInteractivite: 'remplisLesBlancs'
                })
              }
            } else {
              ...
              if (this.interactif) {
                texte += ajouteQuestionMathlive({
                  ...
                  objetReponse: { champ1: { value: String(k * y) }, champ2: { value: String(k * z) }, champ3: { value: String(k * (x + y + z)) }, bareme: toutAUnPoint },
                  typeInteractivite: 'remplisLesBlancs'
                })
              }
            }
        ...
        case 'dilution':
          ...
          if (n === 2) {
            ...
            if (choice([true, false])) {
              texte += 'À quel pourcentage de produit concentré dans le mélange final correspond ce ratio ? (arrondir à l\'unité).<br>'
              ...
              if (this.interactif) {
                texte += ajouteQuestionMathlive({
                  exercice: this,
                  question: i,
                  objetReponse: { reponse: { value: p1, compare: approximatelyCompare, options: { tolerance: 0.5 } } },
                  texteApres: '%',
            ...
            if (choice([true, false])) {
              texte += 'Donner les pourcentages minimum de produit concentré dans le mélange final (arrondi à l\'unité).<br>'
              texteCorr += `Une dilution selon le ratio $~${x}~:~${y}~$ signifie qu'on dilue $${x}$ unités de volume de ${produits[index % 5]} dans $${y}$ unités de volume d'eau.<br>`
              ...
              if (this.interactif) {
                texte += ajouteQuestionMathlive({
                  exercice: this,
                  question: i,
                  content: '\\text{pourcentage minimum : }%{champ1}\\%\\text{ ;  pourcentage maximum : }%{champ2}\\%',
                  objetReponse: { champ2: { value: Math.round(100 * x / (x + y)), compare: approximatelyCompare, options: { tolerance: 0.5 } }, champ1: { value: Math.round(100 * x / (x + z)), compare: approximatelyCompare, options: { tolerance: 0.5 } }, bareme: toutAUnPoint },
                  typeInteractivite: 'remplisLesBlancs'
                })
              }
            } else {
              ...
              if (this.interactif) {
                texte += ajouteQuestionMathlive({
                  ...
                  objetReponse: { champ1: { value: String(k * x) }, champ2: { value: String(k * y) }, champ3: { value: String(k * z - k * y) }, bareme: toutAUnPoint },
                  typeInteractivite: 'remplisLesBlancs'
                })
              }
            }
          ...
          if (n === 3) {
            ...
            if (this.interactif) {
              texte += ajouteQuestionMathlive({
                exercice: this,
                question: i,
                content: '\\text{masse de farine : }%{champ1}\\text{ g}\\text{ ; masse de sucre : }%{champ2}\\text{ g}\\text{ ;  masse de sable : }%{champ3}\\text{ g}',
                objetReponse: { champ1: { value: String(k * x) }, champ2: { value: String(k * y) }, champ3: { value: String(k * z - k * y) }, bareme: toutAUnPoint },
                typeInteractivite: 'remplisLesBlancs'
              })
            }
            ...
            if (this.interactif) {
              texte += ajouteQuestionMathlive({
                exercice: this,
                question: i,
                content: '\\text{volume de vinaigre : }%{champ1}\\text{ mL}\\text{ ; volume de vinaigrette : }%{champ2}\\text{ mL}',
                objetReponse: { champ1: { value: String(k * x * 15) }, champ2: { value: String(k * 15 * (x + y)) }, bareme: toutAUnPoint },
                typeInteractivite: 'remplisLesBlancs'
              })
            }
          }
        ...
        case 'ecran':
          ...
          if (a / x === b / y) {
            ...
            if (this.interactif) {
              texte += `Si oui, saisir $${texNombre(b, 0)}$ dans la zone de saisie, sinon saisir la hauteur adaptée.<br>` + ajouteQuestionMathlive({
                exercice: this,
                question: i,
            ...
            if (Number.isInteger(a / x)) {
              ...
              if (!this.interactif) {
                ...
                texte += `Si oui, saisir $${texNombre(a, 0)}$ dans la zone de saisie, sinon saisir la largeur adaptée.<br>` + ajouteQuestionMathlive({
                  exercice: this,
                  question: i,
              ...
              const callback = (exercice: Exercice, question: number) => {
                ...
                if (prompts.length !== 2) {
                  return { isOk: false, feedback: 'erreur dans le programme : Les zones n\'ont pas été trouvées', score: { nbBonnesReponses: 0, nbReponses: 1 } }
                }
                const [largeur, hauteur] = prompts.map((prompt) => mfe.getPromptValue(prompt))
                const isOk = Number(largeur) > 1024 && (Number(largeur) / x) === (Number(hauteur) / y)
              ...
              if (this.interactif) {
                texte += `Si oui, recopier les nombres $${texNombre(a, 0)}$ et $${texNombre(b, 0)}$ dans les zones de saisies, sinon, saisissez une résolution adaptée avec une largeur supérieure à 1024 pixels` + ajouteQuestionMathlive({
                  exercice: this,
                  question: i,
                  content: ' %{champ1}\\times %{champ2}',
                  objetReponse: { champ1: { value: c.toFixed(0) }, champ2: { value: (c * y / x).toFixed(0) }, bareme: toutAUnPoint, callback },
                  typeInteractivite: 'remplisLesBlancs'
                })
              } else {
                texte += 'Sinon proposer une résolution adaptée à ce ratio avec une largeur supérieure à 1024 pixels.'
              }
