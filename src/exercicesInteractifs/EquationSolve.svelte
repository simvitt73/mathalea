<script>
   ...
   // Son accès est défini dans uuidsRessources.json
    ...
    let types = Object.values(ChangeTypes).sort((a, b) =>
      a.localeCompare(b)
    );
    ...
    function valider() {
      ...
      resultat2 = katex.renderToString(`\\begin{aligned}\n${resu.printSteps.join('\\\\\n')}\n\\end{aligned}`);
  ...
  <br>
  <form on:submit|preventDefault={valider} style="display:flex; gap:0.5rem; align-items:center;">
  <input
    type="text"
    bind:value={saisie}
...
<div style="display:grid; grid-template-columns: 1fr 1fr; gap:0.5rem; max-width:600px;">
  ...
  <label>
    ...
    <input type="text"
      on:input={(e) => options.changeType = e.currentTarget.value.split(',').map(x => x.trim()).filter(Boolean)} />
  </label>
