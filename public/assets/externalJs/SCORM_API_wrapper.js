...
pipwerks.SCORM.API.get = function () {
  let API = null
  const win = window
  const find = pipwerks.SCORM.API.find
  const trace = pipwerks.UTILS.trace
...
pipwerks.SCORM.connection.initialize = function () {
  ...
  const debug = pipwerks.SCORM.debug
...
pipwerks.SCORM.connection.terminate = function () {
  let success = false
  const scorm = pipwerks.SCORM
  const exitStatus = pipwerks.SCORM.data.exitStatus
  const completionStatus = pipwerks.SCORM.data.completionStatus
  const trace = pipwerks.UTILS.trace
  const makeBoolean = pipwerks.UTILS.StringToBoolean
  const debug = pipwerks.SCORM.debug
...
pipwerks.SCORM.debug.getCode = function () {
  const API = pipwerks.SCORM.API.getHandle()
  const scorm = pipwerks.SCORM
  const trace = pipwerks.UTILS.trace
  let code = 0
